...
test("selects every published native package", () => {
  ...
  assert.equal(packageFor("linux", "arm64"), "quarters-cli-linux-arm64");
  assert.equal(packageFor("linux", "x64"), "quarters-cli-linux-x64");
});

test("rejects targets that have no published native package", () => {
  assert.equal(packageFor("freebsd", "arm64"), undefined);
  assert.match(unsupportedPlatform("freebsd", "arm64"), /does not publish/);
});
