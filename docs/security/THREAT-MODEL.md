...
| Home-view source or target changes during setup | Both owned directories remain open; Linux `open_tree` clones the source descriptor and `move_mount` attaches it to the target descriptor before the mounted inode is verified |
